      ...
      {CARE_VISITS.map((visit) => (
        <View key={visit.id} style={{
          ...
          <View style={{ backgroundColor: colors.mist, borderRadius: 14, padding: 10 }}>
            <Text style={{ fontSize: 14, color: colors.ink, fontWeight: '600', fontFamily: 'Nunito-SemiBold' }}>{visit.notes}</Text>
          </View>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
            {Object.entries(visit.handover).slice(0, 4).map(([key, val]) => (
              <View key={key} style={{ width: '48%', backgroundColor: colors.mist, borderRadius: 10, padding: 8 }}>
                <Text style={{ fontSize: 11, color: colors.pewter, fontWeight: '700', fontFamily: 'Nunito-Bold', textTransform: 'capitalize' }}>{key}</Text>
                <Text style={{ fontSize: 12, fontWeight: '700', fontFamily: 'Nunito-Bold', color: colors.ink }} numberOfLines={2}>{val}</Text>
              </View>
            ))}
          </View>
        </View>
