...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!deviceId && body.device_label) {
      const { data: device, error: deviceError } = await db.from('wearable_devices').insert({ elder_id: userId, device_type: body.device_type ?? 'phone', label: body.device_label, vendor: body.vendor, last_seen_at: new Date().toISOString(), battery_pct: body.battery_pct }).select().single();
      if (deviceError) throw deviceError;
      deviceId = device?.id;
    }
    const { data: event, error } = await db.from('wandering_events').insert({ elder_id: userId, wearable_device_id: deviceId, event_type: body.event_type, location_event_id: body.location_event_id, family_notified: ['safe_zone_exit','night_exit','no_response'].includes(body.event_type) }).select().single();
    if (error) throw error;
    if (event.family_notified) {
      const { data: family } = await db.from('family_relationships').select('family_member_id').eq('elder_id', userId).eq('elder_consented', true).eq('is_active', true).eq('notify_on_safe_zone_exit', true);
      await Promise.all((family ?? []).map((f) => dispatchNotification({ recipient_id: f.family_member_id, elder_id: userId, notification_type: 'veilige_zone_verlaten', title_nl: 'Oriëntatiehulp nodig', title_en: 'Orientation support may be needed', body_nl: 'HAVEN zag een dwaal- of nachtgebeurtenis. Bel rustig even mee.', body_en: 'HAVEN saw a wandering or night event. Please calmly check in.', data: { wandering_event_id: event.id } })));
    }
